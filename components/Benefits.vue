<template> 
  <div class="py-[17rem] text-black">
    <div class="b-wrapper">
      <h2 class="max-w-[73rem] mb-[8rem] text-gradient m-auto text-center">Benefits of {{agencyName}} Services</h2>
      <div class=""> 
            ...
            <div class="border border rounded-[1.5rem] border-[#8D9CDA] border-solid px-[5rem] py-[3rem]"> 
              <div v-for="(_benefit, _idx) in benefit" class="mb-[3rem]" :key="_idx" > 
                <div class="py-[2.3rem] px-[1.6rem]">
                  <p class="text-[--blue-100] md font-bold mb-[1rem]">{{ _benefit.title }}</p>
                  <p style="font-size: var(--size-20);" class=" tracking-tighter">{{ _benefit.description }}</p>
                </div>
              </div>
            </div>
          </template>
